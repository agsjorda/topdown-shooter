> **Note (2026-06):** This document predates the modularization. The enum types it mentions (`EquipmentSlotType`, `ItemType`, `ArmorType`) were replaced by ScriptableObject assets (`EquipmentSlotTypeSO`, `ItemCategorySO`), equipment slots are now declared via `InventoryUIConfig.equipmentSlotBindings`, and slot pointer events are delegated to the slots container. See `../../Documentation/QuickStart.md` for current usage; the transaction architecture described below is unchanged.
